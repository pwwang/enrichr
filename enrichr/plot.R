...
if (length(args) < 6) {
    cat(paste("Usage: plot.R <enrichr result file> <output file> <query size>",
...
erdata <- read.table(erfile, header = TRUE, row.names = NULL,
...
ovdata <- t(as.data.frame(strsplit(as.character(erdata$Overlap),
                                   "/", fixed = T)))
pdata <- data.frame(
...
max_ov_n <- max(pdata$OverlapN)
max_log_pval <- max(pdata$LogPval, 1)
pdata$LogPval <- pdata$LogPval / max_log_pval * max_ov_n
pcut <- -log10(.05) * max_ov_n / max_log_pval
pvalticks <- ceiling(max(max_log_pval, -log10(.05)))
if (pvalticks >= 6) {
...
library(ggplot2)
p <- ggplot(pdata) +
    geom_col(aes(x = Term, y = OverlapN), fill = "blue",
             color = "#4286f4", alpha = .1) +
    geom_hline(yintercept = pcut, color = "red",
               alpha = .3, linetype = "dashed") +
    geom_line(aes(x = Term, y = LogPval, group = 1),
              color = "red", size = .5) +
    geom_point(aes(x = Term, y = LogPval, group = 1),
               fill = "white", color = "red3", size = 2, shape = 21) +
    geom_text(aes(x = Term, y = 0, label = TermN, group = 1),
              hjust = -.2, size = 3) +
    scale_y_continuous(
        name = sprintf("# overlapping genes (query size: %s)", qsize),
        sec.axis = sec_axis(
            ~ . * max_log_pval / max_ov_n,
            name = "-log10(adj.pval)",
            breaks = ptbreaks,
            labels = ptlabels
        )
